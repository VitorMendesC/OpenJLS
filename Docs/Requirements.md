...
Every code segment and written requirement below was taken **verbatim from ITU-T T.87 (ISO/IEC 14495-1)**. Each one is implemented by a single RTL file under [`Sources`](../Sources); the RTL file name matches the code segment or written requirement it implements (e.g. `A4_quantization_gradients.vhd` implements Code segment A.4, `A4_1_quant_gradient_merging.vhd` implements Written requirement A.4.1). Written requirements are labelled, in this project, as a child of the code segment that originated them (e.g. `A.4.1` under `A.4`) — an organizational choice, not part of T.87.
