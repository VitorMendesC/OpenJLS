...
\begin{center}
  \includegraphics[width=0.92\textwidth]{OpenJLS_arch.png}
\end{center}
...
\texttt{iRst} is high: present \texttt{iImageWidth}/\texttt{iImageHeight} and
hold them stable across a reset pulse, and the values latch into the internal
\texttt{sImageWidth}/\texttt{sImageHeight} registers on the next clock. Pulse
reset again before streaming a new resolution. Tying the dimension ports to
\texttt{0} selects the \texttt{MAX\_IMAGE\_*} maxima; an out-of-range value
falls back to the maximum with a simulation warning.
...
(speed grade $-1$), Vivado 2025.2, 12-bit, RTL-only (no floorplanning).
$f_{max}$ is true fmax (over-constrained until timing fails). At one
pixel/clock, $\sim$250\,MHz is $\sim$250\,Mpixel/s.
...
Verified by simulation (NVC) with a layered suite: per-module OSVVM tests
against T.87-derived reference models, top-level control-plane stress with
requirements tracking, embedded PSL protocol contracts, 99\%+ NVC statement
coverage, and byte-exact comparison against the CharLS reference encoder over
287 images. The OSVVM stress test and a golden-model subset are re-run on the
post-synthesis gate-level netlist. Live results:
