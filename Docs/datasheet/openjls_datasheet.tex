...
The core never stalls --- neither at EOI (end of image) nor at EOL (end of line) --- which allows processing multiple images back-to-back, with no bubbles.
...
registers on the rising edge while \texttt{iRst} is high
(Figure~\ref{fig:timing-reset}). \texttt{iRst} is level-sensitive --- hold it
high for at least one clock --- and also clears the pipeline. No reset is needed
between images: with the dimensions unchanged, images encode back-to-back. A
reset is needed only to capture the dimensions --- before the first image (they
latch only during reset) and again whenever the image size changes.
