...
--
--            * s_axis : pixel input. One pixel per beat, right-justified in a
...
--
--            * m_axis : encoded .jls byte stream. The core packs the first
...
--              reset owns configuration. See openjls_axi for the AXI-Lite
--              flavor that wraps this contract in registers.
--
-------------------------------------------------------------------------------------------------------------
...
-- AXI port names must keep the s_axis_*/m_axis_* convention (lowercase,
-- underscores) for Vivado IP integrator interface inference.
-- vsg_off port_010
...
entity openjls_axis is
  ...
  attribute x_interface_parameter of iClk      : signal is "ASSOCIATED_BUSIF s_axis:m_axis, ASSOCIATED_RESET iRst";
...
begin
  ...
  u_openjls_top : entity work.openjls_top(rtl)
    ...
    port map (
      iClk             => iClk,
      iRst             => iRst,
      iValid           => s_axis_tvalid,
      iPixel           => s_axis_tdata(BITNESS - 1 downto 0),
      oReady           => s_axis_tready,
      iImageWidth      => iImageWidth,
      iImageHeight     => iImageHeight,
      oData            => sCoreData,
      oValid           => m_axis_tvalid,
      oKeep            => sCoreKeep,
      oLast            => m_axis_tlast,
      iReady           => m_axis_tready
    );
  ...
  gen_byte_swap : for i in 0 to OUT_WIDTH / 8 - 1 generate
    m_axis_tdata((i + 1) * 8 - 1 downto i * 8) <= sCoreData(OUT_WIDTH - i * 8 - 1 downto OUT_WIDTH - (i + 1) * 8);
    m_axis_tkeep(i)                            <= sCoreKeep(OUT_WIDTH / 8 - 1 - i);
