...
entity byte_stuffer is
  generic (
    IN_WIDTH     : natural := CO_BYTE_STUFFER_IN_WIDTH;
    OUT_WIDTH    : natural := CO_BYTE_STUFFER_IN_WIDTH;
    BUFFER_WIDTH : natural := 2 * CO_BYTE_STUFFER_IN_WIDTH + CO_BYTE_STUFFER_IN_WIDTH / 8 -- 2 words buffer + worst-case stuffing expansion (1 bit per byte)
  );
  port (
    iClk        : in std_logic;
    iRst        : in std_logic;
