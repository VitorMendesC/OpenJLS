...
entity tb_jls_framer is
end entity tb_jls_framer;
...
architecture bench of tb_jls_framer is
  ...
  constant IMAGE_WIDTH            : unsigned(log2ceil(MAX_IMAGE_WIDTH + 1) - 1 downto 0)  := to_unsigned(W, log2ceil(MAX_IMAGE_WIDTH + 1));
  constant IMAGE_HEIGHT           : unsigned(log2ceil(MAX_IMAGE_HEIGHT + 1) - 1 downto 0) := to_unsigned(H, log2ceil(MAX_IMAGE_HEIGHT + 1));
  signal   iEoi                   : std_logic;
  signal   iBsWord                : std_logic_vector(IN_WIDTH - 1 downto 0);
  signal   iBsWordValid           : std_logic;
  signal   iBsValidBytes          : unsigned(log2ceil(IN_WIDTH / 8 + 1) - 1 downto 0);
  ...
  begin
    ...
    case idx is
      ...
      when 7 =>
        ...
        return std_logic_vector(to_unsigned(H / 256, 8));

      when 8 =>

        return std_logic_vector(to_unsigned(H mod 256, 8));

      when 9 =>

        return std_logic_vector(to_unsigned(W / 256, 8));

      when 10 =>

        return std_logic_vector(to_unsigned(W mod 256, 8));
  ...
  end function expected_header_byte;

  -- Shared monitor state
  shared variable monByteIdx      : integer;
  shared variable monPayloadBytes : natural;
  shared variable monLastSeen     : boolean;
  shared variable monImageBytes   : integer;                  -- frozen at oLast for post-image check
...
begin
  ...
  dut : entity work.jls_framer(behavioral)
    ...
    port map (
      ...
      iRst             => iRst,
      iStart           => iStart,
      iImageWidth      => IMAGE_WIDTH,
      iImageHeight     => IMAGE_HEIGHT,
      iEoi             => iEoi,
      iWord            => iBsWord,
      iValid           => iBsWordValid,
    ...
    if rising_edge(iClk) then
      if (oWordValid = '1' and iReady = '1') then
        ...
        if (oLast = '1') then
          monLastSeen   := true;
          monImageBytes := bi;
          monByteIdx    := 0;                                                                         -- ready for next image's first beat (T_last+1)
        else
          monByteIdx := bi;
        end if;
      end if;
  ...
  stim : process is
    ...
    begin
      ...
      loop
        ...
        wait until rising_edge(iClk);
        exit when monLastSeen;

      end loop;

      monLastSeen := false;

    end procedure wait_image_done;
  ...
  begin
    ...
    check(monImageBytes = HEADER_LEN + PAYLOAD_BYTES + 2,
          "Test 1: wrong total byte count " & integer'image(monByteIdx));
    report "Test 1 done";
    ...
    check(monImageBytes = HEADER_LEN + 2,
          "Test 2: image 2 wrong byte count " & integer'image(monByteIdx));
    report "Test 2 done";
    ...
    check(monImageBytes = HEADER_LEN + 2,
          "Test 3: wrong byte count " & integer'image(monByteIdx));
    report "Test 3 done";
    ...
    if (errCount > 0) then
      report "tb_jls_framer RESULT: FAIL (" & natural'image(errCount) & " errors)"
        severity failure;
    else
      report "tb_jls_framer RESULT: PASS"
        severity note;
    end if;
