...
architecture bench of tb_openjls_top is
  ...
  -- Internal signals
  signal sPulseIReady            : std_logic;

  -- Collection
  shared variable collected      : byte_array_t(0 to 511);
  shared variable collectedCount : natural;
  shared variable lastCount      : natural;          -- # of oLast pulses seen
...
begin
  ...
  end process pulse_iready;

  -- Stimulus
  stim : process is

    variable baseCollected : natural;
    variable baseLast      : natural;
    ...
    end procedure feed_image_bp;

    procedure wait_n_images (
      n : natural
    ) is
    begin
      ...
      for i in 0 to 9999 loop
        ...
        exit when lastCount >= baseLast + n;
        wait until rising_edge(iClk);

      end loop;

    end procedure wait_n_images;
  ...
  begin
    ...
    do_reset;
    baseCollected := collectedCount;
    baseLast      := lastCount;
    feed_image;
    wait_n_images(1);

    check(collectedCount - baseCollected = EXPECTED_BYTES,
          "Test 1 byte count mismatch: got " & integer'image(collectedCount - baseCollected) &
          " expected " & integer'image(EXPECTED_BYTES));

    for i in 0 to EXPECTED_BYTES - 1 loop

      if (baseCollected + i < collectedCount) then
        check(collected(baseCollected + i) = EXPECTED(i),
              "Test 1 byte " & integer'image(i) &
              " mismatch: exp=" & hex2(EXPECTED(i)) &
              " got=" & hex2(collected(baseCollected + i)));
      end if;
    ...
    check(collectedCount - baseCollected = 2 * EXPECTED_BYTES,
          "Test 2 byte count mismatch: got " & integer'image(collectedCount - baseCollected) &
          " expected " & integer'image(2 * EXPECTED_BYTES));

    for i in 0 to 2 * EXPECTED_BYTES - 1 loop

      if (baseCollected + i < collectedCount) then
        check(collected(baseCollected + i) = EXPECTED(i mod EXPECTED_BYTES),
    ...
    check(collectedCount - baseCollected = 3 * EXPECTED_BYTES,
          "Test 3 byte count mismatch: got " & integer'image(collectedCount - baseCollected) &
    ...
    if (errCount > 0) then
      ...
      report "tb_openjls_top RESULT: PASS"
        severity note;
    end if;
